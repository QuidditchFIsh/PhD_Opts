...
for i in opt_freq:
    ...
    filename = "Freq_out/Frequencies " + str(round(i[0],3)) + "," + str(round(i[0],3)) + "," + str(round(i[0],3)) + "," \
               + str(round(i[0],3)) + ".dat"
    with open(filename, "w") as f:
        f.write(str(i[0]) + "\n")
        for j in range(0,len(Index0)):
            f.write("\t" + str(Capaticance[Index0[j]]) + " "
                    + str(Josephson_Energy[Index0[j]]) + " " + str(frequencies[Index0[j]]) + "\n")
        f.write(str(i[1]) + "\n")
        for j in range(0,len(Index1)):
            f.write("\t" + str(Capaticance[Index1[j]]) + " "
                    + str(Josephson_Energy[Index1[j]]) + " " + str(frequencies[Index1[j]]) + "\n")
        f.write(str(i[2]) + "\n")
        for j in range(0,len(Index2)):
            f.write("\t" + str(Capaticance[Index2[j]]) + " "
                    + str(Josephson_Energy[Index2[j]]) + " " + str(frequencies[Index2[j]]) + "\n")
        f.write(str(i[3]) + "\n")
        for j in range(0,len(Index3)):
            f.write("\t" + str(Capaticance[Index3[j]]) + " "
                    + str(Josephson_Energy[Index3[j]]) + " " + str(frequencies[Index3[j]]) + "\n")
